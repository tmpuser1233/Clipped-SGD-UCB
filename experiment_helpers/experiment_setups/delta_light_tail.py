...
def main():
    ...
    n_trials = 300
    n_actions = 2
    deltas = np.linspace(0.0, 1.0, 25)

    rez = {}
    for delta in deltas:
        reward_arr = np.array([0.0, delta])
        env = environments.NormalDistributionEnv(reward_arr=reward_arr, sigma=1.0)
        R = delta
        agent_list = [
